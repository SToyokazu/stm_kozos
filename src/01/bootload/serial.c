...
#define GPIO_MODER_MASK     (3UL)
#define GPIO_MODER_AFMODE   (2UL)
#define GPIOA_MODER_PA2_MASK   (GPIO_MODER_MASK << (2 * 2))
#define GPIOA_MODER_PA2_AFMODE (GPIO_MODER_AFMODE << (2 * 2))
#define GPIOA_MODER_PA3_MASK   (GPIO_MODER_MASK << (3 * 2))
#define GPIOA_MODER_PA3_AFMODE (GPIO_MODER_AFMODE << (3 * 2))

#define GPIO_OSPEEDR_MASK (3UL)
#define GPIO_OSPEEDR_HIGH (3UL)
#define GPIOA_OSPEEDR_PA2_MASK (GPIO_OSPEEDR_MASK << (2 * 2))
#define GPIOA_OSPEEDR_PA2_HIGH (GPIO_OSPEEDR_HIGH << (2 * 2))
#define GPIOA_OSPEEDR_PA3_MASK (GPIO_OSPEEDR_MASK << (3 * 2))
#define GPIOA_OSPEEDR_PA3_HIGH (GPIO_OSPEEDR_HIGH << (3 * 2))

#define GPIO_AFRL_MASK (15UL)
#define GPIO_AFRL_AF7  (7UL)
#define GPIOA_AFRL_PA2_MASK (GPIO_AFRL_MASK << (2 * 4))
#define GPIOA_AFRL_PA2_AF7  (GPIO_AFRL_AF7 << (2 * 4))
#define GPIOA_AFRL_PA3_MASK (GPIO_AFRL_MASK << (3 * 4))
#define GPIOA_AFRL_PA3_AF7  (GPIO_AFRL_AF7 << (3 * 4))
...
int serial_init(int index)
{
    ...
    STM32F401_GPIOA->MODER &= ~GPIOA_MODER_PA2_MASK;
    STM32F401_GPIOA->MODER |=  GPIOA_MODER_PA2_AFMODE;

    STM32F401_GPIOA->OSPEEDR &= ~GPIOA_OSPEEDR_PA2_MASK;
    STM32F401_GPIOA->OSPEEDR |= GPIOA_OSPEEDR_PA2_HIGH;

    STM32F401_GPIOA->AFRL &= ~GPIOA_AFRL_PA2_MASK;
    STM32F401_GPIOA->AFRL |=  GPIOA_AFRL_PA2_AF7;

    // PA3: USART2_RX, alternate function 7
    STM32F401_GPIOA->MODER &= ~GPIOA_MODER_PA3_MASK;
    STM32F401_GPIOA->MODER |=  GPIOA_MODER_PA3_AFMODE;

    STM32F401_GPIOA->OSPEEDR &= ~GPIOA_OSPEEDR_PA3_MASK;
    STM32F401_GPIOA->OSPEEDR |= GPIOA_OSPEEDR_PA3_HIGH;

    STM32F401_GPIOA->AFRL &= ~GPIOA_AFRL_PA3_MASK;
